...
function showSearchResult() {
    ...
    if ($("#search-by-1").text().indexOf('名称') != -1) {
        ...
        post_data['keyword'] = $("input[name='search-input']").val();
    } else {
    ...
    $.ajax({
        type: 'POST',
        url: "http://127.0.0.1:8000/query/medic_search/",
        data: JSON.stringify(
            post_data
        ),
        dataType: 'json',
...
function showMedicDetails(obj) {
    ...
    $.ajax({
        type: 'POST',
        url: "http://127.0.0.1:8000/query/medic_details/",
        data: JSON.stringify(
            post_data
        ),
        dataType: 'json',
            ...
            $.ajax({
                ...
                success: function (html) {
                    ...
                    for (let d = 0; d < response['cures'].length; d++) {
                        doc.find("#illnesses").append("<span class='label label-default'>" + response['cures'][d] + "</span>\n");
                    }
                    for (let d = 0; d < response['ingredients'].length; d++) {
                        doc.find("#ingredients").append("<span class='label label-default'>" + response['ingredients'][d] + "</span>\n");
                    }
